...
class TradingEnvironment(Environment):
    ...
    def execute(self, action):
        ...
        """
        if (action == Side.Buy):
            self.cash_value = self.__portfolio.cash
            _, _, _, self.current_price, _ = self.__OHLCV.current()
            self.quantity = int(self.cash_value / self.current_price)
        elif (action == Side.Sell):
            self.quantity = self.__portfolio.position(self.__sym)
            pass

        ### time rolls to next period, e.g. next day, then place order
        self.__OHLCV.next()
        self.__sma10.next()
        self.__oms.place(self.__sym, action, self.quantity, None)
